...
1. `receive()`関数を見ると、送られる Ether の量が`prize`よりも多くなければならないという条件があります。したがって、`prize`の値を取得しましょう。
...
- 次に、Ether の送金後に`payable(king).transfer(msg.value);`がトリガーされるはずです。
...
4. `Attack`は Ethernaut 上で作成されたコントラクトインスタンスのアドレスを入力として取ります。送信する Ether の量、`1000000000000000`も`Value`に入力する必要があります。
